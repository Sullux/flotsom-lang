...
describe('lex', () => {
  ...
  it('should lex an xor', () =>
    deepStrictEqual(
      lex('{ foo | bar }')[4],
      {
        name: 'xor', offset: 6, length: 1, position: { x: 7, y: 1 }
      }
    ))
  ...
  it('should lex a pair', () =>
    deepStrictEqual(
      lex('{ foo: bar }')[3],
      {
        name: 'pair', offset: 5, length: 1, position: { x: 6, y: 1 }
      }
    ))

  // eval
  it('should lex an eval', () =>
    deepStrictEqual(
      lex('{ foo ~bar }')[4],
      {
        name: 'eval', offset: 6, length: 1, position: { x: 7, y: 1 }
      }
    ))
})
